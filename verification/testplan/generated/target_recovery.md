...
##### `virtual_read_illegal`

Test: `virtual_read_illegal`

Sets the TTI and recovery addresses via two SETDASA CCCs. Disables
the recovery mode.

Issues a recovery read transfer to read INDIRECT_FIFO_STATUS register.
Checks if the target responded with a NACK to the transfer as the
register is readable only when the recovery mode is enabled in
the target.
